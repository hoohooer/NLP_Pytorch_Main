...
from parsedata.config import Args
import json
import pandas as pd
import time
import re
...
class Trainer():
    ...
    def dev(self):
        ...
        with torch.no_grad():
            for dev_data in self.dev_loader:
                ...
                if args.task_type == "tc" and args.task_type_detail == "singlelabel":
                    outputs = nn.functional.softmax(outputs).cpu().detach().numpy().tolist()
                    outputs = [[1 if j == np.argmax(outputs[i]) else 0 for j in range(len(outputs[i]))] for i in range(len(outputs))]
        ...
        macro_accuracy, micro_accuracy, f1 = getresult(dev_outputs, dev_targets)
