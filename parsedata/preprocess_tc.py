...
def convert_bert_example(args, example: InputExample, tokenizer: BertTokenizer, label2id):
...
def convert_examples_to_features(args, examples, label2id):
...
def preprocess_tc(args, data_all):  # 文本分类任务预处理程序
    ...
    out = convert_examples_to_features(args, examples, label2id)
